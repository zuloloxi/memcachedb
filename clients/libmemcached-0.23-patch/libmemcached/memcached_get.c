...
memcached_return memcached_rget(memcached_st *ptr, 
                                const char *start_key, 
                                size_t start_key_length,
                                const char *end_key, 
                                size_t end_key_length,
                                int is_left_open, 
                                int is_right_open, 
                                unsigned int max_items)
{
  unsigned int x;
  memcached_return rc= MEMCACHED_NOTFOUND;
  char *get_command= "rget ";
  uint8_t get_command_length= 5;
  char postfix[16];
  ...
  /* verify start and end key */
  if ((ptr->flags & MEM_VERIFY_KEY) && (memcachd_key_test((char **)&start_key, &start_key_length, 1) == MEMCACHED_BAD_KEY_PROVIDED))
    return MEMCACHED_BAD_KEY_PROVIDED;
  if ((ptr->flags & MEM_VERIFY_KEY) && (memcachd_key_test((char **)&end_key, &end_key_length, 1) == MEMCACHED_BAD_KEY_PROVIDED))
    return MEMCACHED_BAD_KEY_PROVIDED;
  ...
  */
  ...
  /* postfix */
  sprintf(postfix, "%c %c %d", is_left_open == 0?'0':'1', is_right_open == 0?'0':'1', max_items);
  ...
  */
  for (x= 0; x < ptr->number_of_hosts; x++)
  {
    ...
    {
      rc= memcached_connect(&ptr->hosts[x]);

      if (rc != MEMCACHED_SUCCESS)
        continue;

      if ((memcached_io_write(&ptr->hosts[x], get_command, get_command_length, 0)) == -1)
      {
        rc= MEMCACHED_SOME_ERRORS;
        continue;
      }
    ...
    if (ptr->prefix_key[0] != 0)
    {
      if ((memcached_io_write(&ptr->hosts[x], ptr->prefix_key, ptr->prefix_key_length, 0)) == -1)
      {
    ...
    /* start key */
    if ((memcached_io_write(&ptr->hosts[x], start_key, start_key_length, 0)) == -1)
    {
      memcached_server_response_reset(&ptr->hosts[x]);
      rc= MEMCACHED_SOME_ERRORS;
      continue;
    ...
    if ((memcached_io_write(&ptr->hosts[x], " ", 1, 0)) == -1)
    {
      memcached_server_response_reset(&ptr->hosts[x]);
      rc= MEMCACHED_SOME_ERRORS;
      continue;
    ...
    /* end key */
    if ((memcached_io_write(&ptr->hosts[x], end_key, end_key_length, 0)) == -1)
    {
      memcached_server_response_reset(&ptr->hosts[x]);
      rc= MEMCACHED_SOME_ERRORS;
      continue;
    ...
    /* write out postfix */
    if ((memcached_io_write(&ptr->hosts[x], postfix, strlen(postfix), 0)) == -1)
    {
      memcached_server_response_reset(&ptr->hosts[x]);
      rc= MEMCACHED_SOME_ERRORS;
      continue;
